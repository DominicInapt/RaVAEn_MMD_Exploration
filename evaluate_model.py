...
#D
#Due to newer versions of tqdm, I believet this is required now.
import tqdm.auto
#D
...
#D this will likely help as well.
from tqdm import tqdm 
...
#Hydra begone I have a dictionary.
...
def main():
    ...
    #You will need to change this address. I made mine absolute to prevent errors.
    # reading the data from the file
    with open('/root/RaVAEn/content/RaVAEn/scripts/cfg_copy.txt') as f:
        data = f.read()
    ...
    js = json.loads(data)
    cfg=js
    #cfg = deepconvert(cfg) Original code, VERY important.
    ...
    #module=module.eval()
    logger=None # Replaces logger with a valid item.
    """
    #Removed to prevent wandb dialogue.
...
def evaluate_model(module, data_module, cfg_evaluation, input_shape, logger, input_transformer, target_transformer):
    ...
    for dataloader_index, dataloader in tqdm(enumerate(dataloaders), total=len(dataloaders)):
        ...
        #data_all = torch.load(f'/root/RaVAEn/data_all{dataloader_index}.pt') #Loads in data from CSV.
        ...
        #Make sure to check that it is loading properly.
        
        for batch in tqdm(dataloader, leave=False, desc='Populating data'):
            batch_size = batch[0].shape[0]
            
            #All print statements are debugging.
            ...
            #data = data_all[n_tiles: n_tiles+batch_size] # For quicker iterations.
            
            
            #In offline mode, I won't be able to get past here. cuda requires an nvidia system.
            #ORI could just skip rec
        ...
        for i in tqdm(range(n_images), desc='Analysing sequence', leave=False):
            ...
            #Thanks to this, we know that we are dealing with a single image here.
            ...
            embs_now = embs_all[i:i+1]#embs
            recons_now = recons[i:i+1]#recons
            embs_win = embs_all[:i]   #embs
            recons_win = recons[:i]   #recons
            
            data_now = data_all[i:i+1]#data
            change_now = changes_all[i:i+1]
            data_win = data_all[:i]   #data
       
            
            
            #The reason all previous are included, is because some methods have a memory and others do not.
            #This memory allows for previous images (if any) to be included in the measurement.
            results = dumb_pool_api(detection_methods, is_vae, embs_now,
                     data_now, recons_now, embs_win, data_win, recons_win)
            #I need to figure out a way to transform a distance thingy into an actual image.
            ...
            for method, result in zip(detection_methods, results):
                ...
                if method.returns=='image':
                    if plot_with_colormaps:
                        ...
                        plt.close()
                        #Sooo, it might be useful to enable this to see the different things.
                        #Figure out how to re-wandb everything.
                ...
                #So invalid pixels are allowed to persist with a value of 2 until here where they are gone.
                # Stores the index of the last valid image.
                #This essentially gathers all valid results from the methods.
        ...
        #Changes in this location is the last valid image in the sequence. AKA, the final say in anomaly or not.
        
        #Soo This value is guaranteed to not have any 2's
        #Soo the true changes is the last changes_all with no 2's that returns an image.
